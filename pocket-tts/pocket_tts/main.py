...
def text_to_speech(
    ...
    if voice_url is not None:
        ...
        suffix = Path(voice_wav.filename).suffix.lower() if voice_wav.filename else ".wav"
        ...
        # Convert non-WAV formats to WAV so soundfile can read them
        ...
        if suffix != ".wav":
            ...
            ffmpeg = next(
                (p for p in ["/opt/homebrew/bin/ffmpeg", "/usr/local/bin/ffmpeg", "ffmpeg"]
                 if os.path.isfile(p) or p == "ffmpeg"),
                "ffmpeg"
            )
            result = subprocess.run(
                [ffmpeg, "-y", "-i", temp_file_path, "-ar", "24000", "-ac", "1", converted_path],
                capture_output=True,
            )
            if result.returncode != 0:
                ...
                raise HTTPException(
                    status_code=422,
                    detail=f"Could not convert audio file: {result.stderr.decode()[:200]}",
                )
            wav_path = converted_path

        try:
            model_state = tts_model.get_state_for_audio_prompt(Path(wav_path), truncate=True)
